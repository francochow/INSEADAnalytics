...
**Your Answers here:**
<br>
We have `r nrow(StockReturns)` days of data, starting from `r rownames(StockReturns)[1]` until `r tail(rownames(StockReturns),1)`.  Here are some basic statistics about the AAPL returns:

1. The cumulative returns of the AAPL index during this period is `r round(100*sum(StockReturns[,2]),1)`%.
2. The average daily returns of the AAPL index during this period is `r round(100*mean(StockReturns[,2]),3)`%;
2. The standard deviation of the daily returns of the AAPL index during this period is `r round(100*sd(StockReturns[,1]),2)`%;

Here are returns of the AAPL in this period (note the use of the helper function pnl_plot - defined in file helpersSet1.R):
...
<br>
<br>
We have `r nrow(StockReturns)` days of data, starting from `r rownames(StockReturns)[1]` until `r tail(rownames(StockReturns),1)`.  Here are some basic statistics about the YHOO returns:

1. The cumulative returns of the YHOO during this period is `r round(100*sum(StockReturns[,3]),1)`%.
2. The average daily returns of the YHOO index during this period is `r round(100*mean(StockReturns[,3]),3)`%;
2. The standard deviation of the daily returns of the YHOO index during this period is `r round(100*sd(StockReturns[,3]),2)`%;

Here are returns of the YHOO in this period (note the use of the helper function pnl_plot - defined in file helpersSet1.R):
...
**Your Answers here:**
<br>
1. Change the date at dataSet1.R line 9 
<br>
From startDate = "2001-01-01"
<br>
To startDate = "2000-12-30"
<br>
<hr>
<hr>
...
**Your Answers here:**
<br>
1. It shows [1] 20, I believe it refers to the number of entries of the data.
<br>
If I run the command sum(myData = StockReturns[1:20,]) instead, it shows 0.8068457, which I believe is the sum of stock return for the first 20 days. 
<br>
<br>
